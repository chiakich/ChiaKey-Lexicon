...
Google Trends 與 PTT 八卦板都只用於「發現訊號」：每日流程收集 Google Trends 的 24 小時、48 小時、7 天視窗，以及八卦板過去 24 小時內淨推文數至少 50 的文章標題。流程會先以 index 頁篩選熱門候選文章，再抓取候選文章的推文數；只保存最小化正規化觀測值為 GitHub Actions artifacts。每週流程再聚合觀測值，只將 ChiaKey 自有的最終覆蓋列寫回本目錄。
...
- 正規化後只保留全漢字詞。
- 移除 ASCII 字母與數字。
- PTT 僅使用文章標題與淨推文數產生候選詞；不保存文章內容、推文內容、作者或 IP。
