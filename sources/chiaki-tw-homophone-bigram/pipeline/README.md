...
#    --rival-evidence 要餵未過門檻的完整配對表，否則對手的真實計數會被當成 0
...
- **重產不會得到一樣的 230,993 列。** `build-corpora.mjs` 是事後重建的，和當初的前處理不完全
  一致（govnews 差 0.07%，ntpc 還沒對上），新聞來源本身也會隨時間變動。`evaluate` 的數字要
  重新量，不要沿用 [../README.md](../README.md) 附錄裡的數值。
